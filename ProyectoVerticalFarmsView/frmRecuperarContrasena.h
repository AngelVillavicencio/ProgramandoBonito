...
#pragma endregion
	...
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		int cantidadUsuariosRegistrados = this->objGestorUsuario->cantidadUsuarios();
		for (i = 0; i < cantidadUsuariosRegistrados; i++) {
			String^ correoLeido = this->objGestorUsuario->verUsuarioDeListaXIndice(i)->getCorreo();
			String^ nombrePadreLeido = this->objGestorUsuario->verUsuarioDeListaXIndice(i)->getPalabraClave();
		...
		if (registrado) {
			textBox3->Text = this->objGestorUsuario->verUsuarioDeListaXIndice(i)->getContrasena();
		}
		else
		{
